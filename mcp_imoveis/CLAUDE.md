...
- Queries are built with `psycopg.sql` composables and run through a `ClientCursor` (client-side param interpolation). `cidade`/`bairro` are normalized tables now (`pierre.cidades`/`pierre.bairros`, referenced by `imoveis.cidade_id`/`imoveis.bairro_id`) — `buscarImoveis` `LEFT JOIN`s them and matches with `unaccent(...) ILIKE unaccent(...)` against `c.nome`/`b.nome` (aliases `c`/`b`). The legacy text columns `imoveis.cidade`/`imoveis.bairro`/`imoveis.uf`/`imoveis.regiao_cidade` were dropped from the database — `cidade`/`bairro` in the `colunas` SELECT list are resolved as `c.nome AS cidade`/`b.nome AS bairro` from the joined tables, not as columns on `imoveis`. The DB must have the `unaccent` extension.
